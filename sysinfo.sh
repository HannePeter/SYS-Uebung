...
echo "ROOT:          total: $(df -h / | grep / | tr -s " " | cut -d" " -f2 )"
echo "               free:  $(df -h / | grep / | tr -s " " | cut -d" " -f4 )"

if df | grep /home; then
    echo "HOME:          total: $(df -h | grep /home | tr -s " " | cut -d" " -f2 )"
    echo "               free:  $(df -h | grep /home | tr -s " " | cut -d" " -f4 )"
fi

echo "Anz. Prozesse: $(ps -A | wc -l)"
